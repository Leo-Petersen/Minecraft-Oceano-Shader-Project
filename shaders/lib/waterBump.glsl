...
float getWaterBump(vec2 posxz, float waveM, float waveZ, float iswater, float viewDist) {
    ...
    #ifdef waveDistanceFade
        // Fade fine detail early (kills the 'shimmer' in distant reflections)
        float fineFade   = smoothstep(18.0, 65.0, viewDist);
        float coarseFade = smoothstep(70.0, 170.0, viewDist);
        mediumWave *= (1.0 - fineFade * 0.65);
        ripples    *= (1.0 - fineFade * 0.90);
        largeWave  *= (1.0 - coarseFade * 0.35);
