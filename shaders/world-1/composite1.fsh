...
vec3 netherNebula(vec3 dir){
    ...
    vec3 neb = mix(vec3(0.22, 0.06, 0.04), vec3(0.24, 0.09, 0.06), fbm(dir.xz * 1.5 - t));
...
void main(){
	...
	float skyUp = clamp(skyDir.y * 0.5 + 0.5, 0.0, 1.0);
	// flat red Nether haze for the sky
	vec3 netherZenith  = vec3(0.13, 0.035, 0.022);
	vec3 netherHorizon = vec3(0.24, 0.070, 0.045);
	vec3 skyBoxCol = mix(netherHorizon, netherZenith, skyUp);

	// Potentially re-add sun and nebula/star effects when above the Nether ceiling? Will re-visit
	vec3  sunDirW = atmSunTrue;
	float sunCos  = dot(skyDir, sunDirW);
	float sunDisc = smoothstep(0.9992, 0.9997, sunCos);
	float sunGlow = pow(max(sunCos, 0.0), 350.0) * 0.35
				+ pow(max(sunCos, 0.0),  40.0) * 0.14;
	vec3  sunTint = vec3(1.00, 0.80, 0.92);
	//skyBoxCol += sunTint * (sunDisc * 8.0 + sunGlow);
	...
	if (Depth < 1.0) {
		...
		if (isEyeInWater < 0.9) {
			// vec3 endSky = skyBoxCol;
			// endSky += netherNebula(skyDir);
			// endSky += netherStars(skyDir);
			// color.rgb = endSky * (1.0 - effects * 0.95) * 1.2;
		}
	}
	...
	#ifdef VolumetricClouds
	{
		...
		if (gl_FragCoord.x < cornerSize.x && gl_FragCoord.y < cornerSize.y) {
			...
			float sampleDepth = texture2D(depthtex0, fullUV).r;
			...
			if (sampleDepth < 1.0) {
				vec4 tClip = vec4(fullUV, sampleDepth, 1.0) * 2.0 - 1.0;
				vec4 tView = gbufferProjectionInverse * tClip; tView /= tView.w;
				terrainDist = length(tView.xyz);
			}

			float dith = vcBayer8(vec2(checkerPos));
			dith = fract(dith + float(frameCounter / area) * 0.61803399);

			cloudLowRes = computeVolumetricClouds(worldDir, terrainDist, dith, cloudSteps, atmSunTrue.y, cloudDist);
			cloudMarched = 1.0;
		}
	}
	#endif
