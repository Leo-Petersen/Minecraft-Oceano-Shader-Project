...
#include "/world1/lib/clouds.glsl"
...
vec3 endNebula(vec3 dir){
    ...
    vec3 neb = mix(vec3(0.35, 0.10, 0.55), vec3(0.05, 0.30, 0.40), fbm(dir.xz * 1.5 - t));
...
void main(){
	...
	vec3 endZenith  = vec3(0.020, 0.010, 0.050);
	vec3 endHorizon = vec3(0.090, 0.055, 0.150);
	vec3 skyBoxCol = mix(endHorizon, endZenith, skyUp) * 2;
	...
	if (Depth < 1.0) {
		...
		if (isEyeInWater < 0.9) {
			...
			endSky += endNebula(skyDir);
			endSky += endStars(skyDir);
			color.rgb = endSky * (1.0 - effects * 0.95);
		}
	}
